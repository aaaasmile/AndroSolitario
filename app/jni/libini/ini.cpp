...
static bool __ini_extractField(ini_t *ini, FILE *file, ini_parser_t &parser,
                               char ch);
 ...
 * Globals Used      :
 * Globals Modified  :
 * Description       : Removes all char deemed to be spaces from start and end
 *of string.
 ********************************************************************************************************************
 *  Rev   |   Date   |  By   | Comment
 * ----------------------------------------------------------------------------------------------------------------
...
void __ini_strtrim(char *str) {
    ...
    // Clip end first
    while (isspace(str[last]) && last > 0) last--;

    // Clip beginning
    while (isspace(str[first]) && (first < last)) first++;
    len = last + 1 - first;
 ...
 *                   : comment - file comment character(s), flags - various
 *operational flags (backup/case/etc) Returns           : Pointer to ini
 *database. Globals Used      : Globals Modified  : Description       : Opens an
 *ini data file and reads it's contents into a database
 ********************************************************************************************************************
 *  Rev   |   Date   |  By   | Comment
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
ini_t *__ini_open(const char *name, ini_mode_t mode, const char *comment,
    ...
    if (!file) {  // File doesn't exist so check if allowed
        // to create new one
        if (mode != INI_NEW)
            goto ini_openError;
    ...
    if (!ini->ftmp)
        goto ini_openError;
    if (file) {  // Process existing ini file
...
ini_openError:
    if (ini) {
        if (ini->ftmp) {  // Close and remove backup file
            fclose(ini->ftmp);
            if (ini->mode != INI_READ) {
 ...
 * Parameters        : ini - pointer to ini file database.  force - if true, the
 *database will be removed from : memory even if an error occured in saving the
 *new ini file. Returns           : -1 on error, 0 on success Globals Used :
 * Globals Modified  :
 * Description       : Save any changes back to the new ini file.
 *                   : The backup file contains all the orignal data + any
 *modifcations appended at the bottom
 ********************************************************************************************************************
 *  Rev   |   Date   |  By   | Comment
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
int __ini_close(ini_t *ini, bool flush) {
    ...
    if (ini->flags & INI_MODIFIED) {
        if (!ini->first)
            ...
            if (file) {  // Output all new headers and keys
    ...
    if (ini->mode != INI_READ) {  // If no mods were made, delete tmp file
        if ((ini->flags & (INI_MODIFIED | INI_NEWFILE | INI_BACKUP)) ^
            (INI_MODIFIED | INI_BACKUP)) {
            ini->filename[strlen(ini->filename) - 1] = '~';
            remove(ini->filename);
        }
    }
 ...
 * Globals Used      :
 * Globals Modified  :
 * Description       : Deletes the whole ini database from memory, but leaves
 *the ini stub so the ini_close can be : called.
 ********************************************************************************************************************
 *  Rev   |   Date   |  By   | Comment
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
void __ini_delete(ini_t *ini) {  // If already deleted, don't delete it again
    if (!ini->first)
        return;
 ...
 * Parameters        : ini    - pointer to ini file database,  file - ini file
 *to read heading from : parser - current parser decoding state, ch   -
 *character to process Returns           : false on error and true on success
 * Globals Used      :
 * Globals Modified  :
...
bool __ini_extractField(ini_t *ini, FILE *file, ini_parser_t &parser, char ch) {
    switch (ch) {  // Check for key value
        case '=':
            if (parser.state !=
                INI_IN_SECTION) {  // Make sure the key has a string content
                parser.last = parser.pos;
                if (parser.first >= 0) {
                    if (!ini->selected)  // Handle keys which are not in a
                                         // section
                    {
 ...
 * Parameters        : ini    - pointer to ini file database,  file - ini file
 *to read heading from : parser - current parser decoding state Returns : false
 *on error and true on success Globals Used      : Globals Modified  :
 * Description       : Comment strings are not passed to ini_extractField. Since
 *multi char comments are now : supported we need re-process it should it
 *eventually turn out not to be a comment.
 ********************************************************************************************************************
 *  Rev   |   Date   |  By   | Comment
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
 ...
 *read heading from Returns           : -1 on error and 0 on success Globals
 *Used      : Globals Modified  : Description       : Read the ini file to
 *determine all valid sections and keys.  Also stores the location of : the keys
 *data for faster accessing.
 ********************************************************************************************************************
 *  Rev   |   Date   |  By   | Comment
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
        ...
        while (count--) {
            ...
            switch (ch) {
                ...
                __ini_processDataEnd:
                    ...
                    if (parser.key) {
                        parser.key->length =
                            (size_t)(parser.pos - parser.key->pos);
                        parser.key = NULL;
                    }
                    break;

                case '\0':
                    // If we get this we are processing a binary or corrupt
                    // file. Drop out here, else we may do something nasty
                    goto __ini_processError;
                        ...
                        default:
                            ...
                            if (parser.state & INI_ALLOW_COMMENT) {
                                if (ch == parser.comment[parser.commentpos]) {
                                    parser.commentpos++;
                                    if (parser.comment[parser.commentpos] ==
                                        '\0') {
                                        parser.commentpos = 0;
                                        parser.state = INI_SKIP_LINE;
 ...
 * Parameters        : ini - pointer to ini file database,  file - ini file to
 *read heading from Returns           : -1 on error and 0 on success Globals
 *Used      : Globals Modified  : Description       : Writes a new ini file
 *containing all the necessary changes
 ********************************************************************************************************************
 *  Rev   |   Date   |  By   | Comment
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
 ...
 * Globals Used      :
 * Globals Modified  :
 * Description       : Opens an ini data file and reads it's contents into a
 *database
 ********************************************************************************************************************
 *  Rev   |   Date   |  By   | Comment
 * ----------------------------------------------------------------------------------------------------------------
 ...
 *name.  key - key name Returns           : Number of bytes to read the keys
 *data in as a string.  1 must be added to this length : to cater for a NULL
 *character. Globals Used      : Globals Modified  : Description       : Number
 *of bytes to read the keys data in as a string
 ********************************************************************************************************************
 *  Rev   |   Date   |  By   | Comment
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
 ...
 * Globals Used      :
 * Globals Modified  :
 * Description       : Copies the contents of the src ini to the dst ini.  The
 *resulting ini contains both : headings and keys from each.  Src keys will
 *overwrite dst keys or similar names.
 ********************************************************************************************************************
 *  Rev   |   Date   |  By   | Comment
 * ----------------------------------------------------------------------------------------------------------------
 ********************************************************************************************************************/
extern "C" int INI_LINKAGE ini_append(ini_fd_t fddst, ini_fd_t fdsrc) {
    ...
    current_h = src->first;
    while (current_h) {  // Locate heading in the dst
        ini_locateHeading(dst, current_h->heading);
        // Go through the src keys under the heading
        src->selected = current_h;
        current_k = current_h->first;
        while (current_k) {  // Check if data buffer can hold the key
            int i = current_k->length;
            current_h->selected = current_k;
            if (i > length) {  // Make data buffer bigger, with some spare
                length = i + 10;
                if (data != NULL)
                    free(data);

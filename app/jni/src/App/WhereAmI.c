    ...
    for (ok = false; !ok; ok = true) {
        ...
        if (size == 0)
            ...
            do {
                ...
                path_ =
                    (wchar_t*)WAI_REALLOC(path, sizeof(wchar_t) * size_ * 2);
                if (!path_)
                    break;
                size_ *= 2;
                path = path_;
        ...
        length__ = WideCharToMultiByte(CP_UTF8, 0, buffer2, length_, out,
                                       capacity, NULL, NULL);

        if (length__ == 0)
            length__ = WideCharToMultiByte(CP_UTF8, 0, buffer2, length_, NULL,
                                           0, NULL, NULL);
        if (length__ == 0)
            break;
            ...
            if (sscanf(buffer,
                ...
                if (low <= addr && addr <= high) {
                    ...
                    if (length > 4 && buffer[length - 1] == 'k' &&
                        ...
                        char* begin = (char*)mmap(0, offset, PROT_READ,
                                                  MAP_SHARED, fd, 0);
                        if (begin == MAP_FAILED) {
                            close(fd);
                            length = -1;  // retry
                        ...
                        char* p = begin + offset -
                                  30;       // minimum size of local file header
                        while (p >= begin)  // scan backwards
                        {
                            if (*((uint32_t*)p) ==
                                0x04034b50UL)  // local file header signature
                                               // found
                            {
        ...
        if (strchr(argv[0], '/')) {
            ...
            while (1) {
                ...
                if (end - begin > 0) {
                    ...
                    if (((end - begin) + 1 + argv0_length + 1) <=
                        sizeof(buffer2)) {
                        memcpy(buffer2, begin, end - begin);
                        buffer2[end - begin] = '/';
                        memcpy(buffer2 + (end - begin) + 1, argv[0],
                               argv0_length + 1);
